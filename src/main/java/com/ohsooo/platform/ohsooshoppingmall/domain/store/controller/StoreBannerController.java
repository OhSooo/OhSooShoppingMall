...
public class StoreBannerController {
    ...
    @Operation(summary = "배너 순서 변경", description = "스토어 owner가 배너의 노출 순서를 변경합니다")
    @PatchMapping("/order")
    ...
    @Operation(summary = "배너 삭제", description = "스토어 owner가 배너를 완전히 삭제합니다")
    ...
    public ResponseEntity<Void> deleteBanner(
        ...
        storeBannerService.deleteBanner(storeId, bannerId, userId);
        return ResponseEntity.noContent().build();
    }

    @Operation(summary = "배너 목록 조회", description = "스토어 배너를 정렬 순서대로 조회합니다")
    @GetMapping
    public ResponseEntity<BaseResponse<List<StoreBannerResponse>>> getBanners(
            @PathVariable Long storeId
    ) {
        List<StoreBannerResponse> response = storeBannerService.getBanners(storeId);
